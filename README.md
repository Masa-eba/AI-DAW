...
23. `M / S / Volume / Pan`でトラックを調整
24. `Loop Clip`で選択中AudioClip / MidiClipの範囲を繰り返し再生
25. `Loop`でプロジェクト長の範囲を繰り返し再生
26. `Panic`または`Esc`で鳴りっぱなしのMIDIノートを停止
27. `Export Mix`でミックス結果を書き出す
