...
14. `Quantize`で選択中MidiClipを16分グリッドへ揃える
15. `Command + ↑ / ↓`で選択中MidiClipをオクターブ上下に移調
16. `Command + ] / [`で選択中MidiClipのVelocityを上下
17. `Zoom`でタイムラインの横方向表示倍率を調整
18. `M / S / Volume`でトラックを調整
19. `Loop Clip`で選択中AudioClip / MidiClipの範囲を繰り返し再生
20. `Loop`でプロジェクト長の範囲を繰り返し再生
21. `Export Mix`でミックス結果を書き出す
