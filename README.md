...
29. `Option + , / .`で前後のマーカーへ移動
30. `M / S / Volume / Pan`でトラックを調整
31. `Loop Clip`で選択中AudioClip / MidiClipの範囲を繰り返し再生
32. `Loop`でプロジェクト長の範囲を繰り返し再生
33. `Panic`または`Esc`で鳴りっぱなしのMIDIノートを停止
34. `Export Mix`でミックス結果を書き出す
